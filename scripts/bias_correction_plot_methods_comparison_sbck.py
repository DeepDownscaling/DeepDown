...
def plot_maps_correl_for_rcm(conf, files, model):
    ...
    # Compute the spearman's rank correlation
    correl_model_clim = xs.spearman_r(ds[vars[0]], ds[vars[1]], dim="time", skipna=True)
    correl_diff = correl_model_clim - correl_ref
    ...
    for row, method in enumerate(methods):
        ...
        for col, data_type in enumerate(files):
            ...
            if os.path.exists(file_path):
                ...
                ax.set_title(f"Difference for {method} ({subtitle})", fontsize=10)
                ax.add_feature(cfeature.COASTLINE)
                ax.add_feature(cfeature.BORDERS, linestyle=":")
    ...
    # Save the figure
    plt.savefig(os.path.join(path_output, f"maps_correl_for_rcm_{model}.png"), dpi=300, bbox_inches="tight")
    plt.savefig(os.path.join(path_output, f"maps_correl_for_rcm_{model}.pdf"), dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved in {path_output}")
